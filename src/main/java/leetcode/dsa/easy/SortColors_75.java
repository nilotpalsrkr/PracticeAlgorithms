...
public class SortColors_75 {
    public void sortColors(Integer[] nums) {
        List<Integer> distinctElements = Arrays.asList(nums).stream().distinct().sorted().collect(Collectors.toList());
        Map<Integer, FunctionIntialIndex> elementIndexMap = new HashMap<>();
        FunctionIntialIndex[] indexArr = new FunctionIntialIndex[distinctElements.size()];
        int lastColor = -1;
        for(int i = 0; i < distinctElements.size() ; i++) {
            ...
            if (i != distinctElements.size() - 1) {
                ...
                elementIndexMap.put(dElement, new FunctionIntialIndex(nums.length - 1, x -> x-1));
                lastColor = dElement;
        ...
        int finalLastColor = lastColor;
        long lastElemntCount = Arrays.asList(nums).stream().filter(x -> x == finalLastColor).count();
        for(int i = 0 ; i < nums.length - lastElemntCount; ) {
